...
function showPage(pageId) {
    ...
    // 2. Find the specific page we want to see
    const targetPage = document.getElementById(pageId);

    // 3. If the page doesn't exist yet, wait 100ms and try again
    if (!targetPage) {
        console.warn("Waiting for HTML to load...");
        setTimeout(() => showPage(pageId), 100);
        return;
    }

    // 4. Hide everyone else
    pages.forEach(p => {
